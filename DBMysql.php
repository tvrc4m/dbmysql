...
class DBMysql {
            ...
            foreach ($where as $name=>$value) {
                ...
                if(is_array($value)){
                    ...
                    if($k==='$like'){
                        ...
                        $sql.=$name.' NOT IN ('.implode(',', $value).') AND ';
                    }else{
    ...
    private function _connect(){
        // 读取config/config.php里面的配置信息
        $option=$GLOBALS['config']['dbOnline'];

        $this->_link=mysqli_connect($option['host'],$option['username'],$option['password'],$option['dbname']);

        if(!$this->_link) throw new Exception(mysqli_connect_error());

        !empty($option['charset']) && $this->charset=$option['charset'];
